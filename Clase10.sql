...
(26, 'Walter', 'White', '44759810', 'Las Vegas 789'),
...
select max(monto) maximo from facturas; -- agrego un alias a la columna
select max(monto) maximo from facturas where letra='b'; -- trae el mayor monto de las
-- facturas con letra 'b'

-- no debe haber un espacio entre el nombre de la función y el primer paréntesis
-- esto puede variar según el motor de la Base de Datos

-- MAX con texto
select * from clientes;
select max(nombre) ultimo_nombre from clientes;
-- trae el úlitmo nombre, teniendo en cuenta el orden alfabético
Select * from facturas;
-- MAX con fechas
...
-- trae la cantidad de registros de la tabla clientes, cuya direccion no sea nula
...
-- SUBQUERIES
select max(monto) from facturas; -- me devuelve 50.000 que es el monto máximo
...
-- sea igual al monto máximo de la tabla
select * from facturas where fecha=(select max(fecha) from facturas);
-- trae todos los campos de todos los registros de la tabla facturas, cuya fecha
-- sea la más reciente
...
describe articulos;
alter table articulos add StockMinimo smallint unsigned not null;
describe articulos;
alter table articulos add stockMaximo smallint unsigned not null;
describe articulos;
...
update articulos set stockMinimo = 100;
update articulos set stockMaximo = 20000;
select * from articulos;
...
select nombre, stockMaximo-stock as reponer
from articulos
...
(1, 'Renault', 'Sandero', 'Gris', '2001', '37000000'),
(2, 'Renault', 'Sandero', 'Blanco', '2023', '70000000'),
(3, 'Renault', 'Sandero', 'Azul', '2018', '52000000'),
(4, 'Peugeot', 'Rifter', 'Roja', '2000', '10500000'),
(5, 'Peugeot', 'Rifter', 'Roja', '2023', '15900000'),
(6, 'Peugeot', 'Rifter', 'Plata', '2016', '800000'),
(7, 'Peugeot', 'Rifter', 'Verde', '2004', '11000000'),
(8, 'Peugeot', 'Rifter', 'Blanca', '2019', '10000000'),
(9, 'Renault', 'Sandero', 'Plata', '2021', '60000000'),
(10, 'Renault', 'Sandero', 'Amarillo', '2022', '65000000'),
(11, 'Peugeot', '208', 'Azul', '2023', '20000000'),
(12, 'Peugeot', '208', 'Negro', '2015', '10900000'),
(13, 'Renault', 'Kangoo', 'Negro', '2022', '15800000'),
(14, 'Renault', 'Kangoo', 'Verde', '2017', '10300000'),
(15, 'Renault', 'Kangoo', 'Blanca', '2026', '9000000');
...
select max(precio) precio_mayor from autos;

-- 	b. obtener el precio mínimo.

select min(precio) precio_menor from autos;
...
select round(avg(precio), 2) precio_promedio_anio_2023 
from autos
where anio = 2023;
...
select anio, count(*) total 
from autos 
group by anio;

-- 	i. obtener la cantida total de autos y el precio promedio en cada año.

select anio, count(*) total, round(avg(precio),2) precio_promedio 
from autos 
group by anio;

-- 	j. obtener la suma de precios y el promedio de precios según marca.

select marca, sum(precio) suma_total, round(avg(precio),2) precio_promedio
from autos 
group by marca;

--  k. informar los autos con el menor precio. 

 select count(*) precio_menor from autos where precio between 0 and 10000000;
...
delete from autos where fecha <= '2000-12-31';